...
This skill has you show commands, outputs, and captured artifacts. Redact every secret first and use `<REDACTED>` in its place. Build loops against environment variables so credentials stay out of commands and output. Captured artifacts may contain authentication headers; quote only the lines that carry the signal.
