...
Use this profile when a repository needs agent-authored charts from small semantic specs rather than backend-native Vega-Lite, ECharts, or Chart.js JSON. Transform data before Flint when aggregation, joins, pivots, or derived columns are needed.
...
Flint MCP renders locally and can read local JSON, CSV, or TSV files by default; remote URLs are not fetched. Use `create_chart_view` when MCP Apps are supported, `render_chart` for static artifacts, and `compile_chart` for backend-native JSON.
...
Use this profile when a repository wants a documented agent preference for lazy senior developer mode. The profile records guidance only: plugin marketplaces, lifecycle hooks, `PONYTAIL_DEFAULT_MODE`, and machine-level harness config remain user-owned. Pair it with `--skill ponytail` for a vendored, on-demand skill.
...
- `opencode-flint-chart-mcp`: adds a project-local `opencode.json` MCP entry for `npx -y flint-chart-mcp --disable-file-reference`.
...
For a local OpenAI-compatible MLX endpoint, run Headroom against the local model server explicitly:

```bash
uv run headroom proxy --openai-api-url http://127.0.0.1:8989/v1
```
